...
class Process:
    ...
    def detect_objects_heatmap(self, heatmap):
        data = 256 * heatmap
        ...
        for (dy, dx) in slices:
            ...
            if heatmap[pos[0], pos[1]] > self.config.CENTER_TR:
                objects[pidx, :] = pos
                pidx += 1
        return objects[:pidx]
    ...
    @staticmethod
    def gaussian_heatmap(h, w, pos_x, pos_y,
                         sigma_h=1, sigma_w=1, init=None):
        """
        Compute the heat-map of size (w x h) with a gaussian distribution
        fit in position (pos_x, pos_y) and a convariance matix defined by
        the related sigma values.
        ...
        tmp = rv.pdf(pos)
        hmap = np.multiply(tmp, np.sqrt(
            np.power(2 * np.pi, 2) * np.linalg.det(cov_matrix)))
    ...
    def prepare_input_posenet(self, image, objects, size_person, size,
                              sigma=25, max_num_objects=16, border=400):
        ...
        for oid, (yc, xc) in enumerate(objects):
            ...
            result[oid, :, :, 3] = self.gaussian_kernel(
                size[0], size[1], sigma, sigma)
        return np.split(result, [3], 3)

    def detect_parts_heatmaps(self, heatmaps, centers, size, num_parts=14):
        """
        ...
        for oid, (yc, xc) in enumerate(centers):
            ...
            for pid in xrange(num_parts):
                ...
                visible[oid, pid] = np.mean(
                    part_hmap[:, :, pid]) > self.config.VISIBLE_PART
        return parts, visible

    @staticmethod
    def import_json(path='json/MPI_annotations.json',
                    order='json/MPI_order.npy'):
        """
        Get the json file containing the dataset.

        We want the data to be shuffled, however the training has
        to be repeatable.
    ...
    def generate_labels(self, image_shape, joint_positions, num_other_people,
                        joints_other_people, offset):
        """
        Given as input a set of joint positions and the size of the input
        image it generates a set of heat-maps of the same size. It generates
        both heat-maps used as labels for the first stage (label_1st_lower)
        and for all the other stages (label_lower)
        """

        _FILTER_JOINTS = np.array(
        ...
        for i in range(self.config.NUM_JOINTS):
            ...
            if not skip:
                heat_maps_single_p[i] = self.gaussian_heatmap(
                ...
                heat_maps_other_p[i] = self.gaussian_heatmap(
                    self.config.INPUT_SIZE, self.config.INPUT_SIZE,
                    curr_joint[1] - offset[1], curr_joint[0] - offset[0],
                    self.config.SIGMA, self.config.SIGMA
                )
        ...
        heat_maps_single_p[-1] = np.maximum(
            ...
            np.zeros((self.config.INPUT_SIZE, self.config.INPUT_SIZE))
        )
        ...
        for p in range(int(num_other_people)):
            for i in range(self.config.NUM_JOINTS):
                # the set of joints can be different fromt the one in the json
                # file
                try:
                    ...
                    skip = (
                        curr_joint[0] < 0 or
                        curr_joint[1] < 0 or
                        curr_joint[0] >= img_width or
                        curr_joint[1] >= img_height)
        ...
        labels_single = self.rescale(
            heat_maps_single_p, self.config.OUTPUT_SIZE)

        labels_people = self.rescale(
            heat_maps_other_p, self.config.OUTPUT_SIZE)

        return labels_people, labels_single

    def generate_center_map(self, center_pos, img_shape):
        """
        Given the position of the person and the size of the input image it
        generates a heat-map where a gaissian distribution is fit in the
        position of the person in the image.
        """
        img_height = img_shape
        img_width = img_shape

        center_map = self.gaussian_heatmap(
            img_height, img_width, center_pos[1], center_pos[0],
            self.config.SIGMA_CENTER, self.config.SIGMA_CENTER)
    ...
    def crop_image(self, image, obj_pose):
        """
        Crop the image in order to have the person at the center and the final
        image size is the same as the expected CNN input size.

        Returns the cropped image and the offset that is used to update the
        joint positions.
        """

        offset_left = int(obj_pose[0] - self.config.INPUT_SIZE / 2)
        offset_up = int(obj_pose[1] - self.config.INPUT_SIZE / 2)

        # just for checking that it's inside the image
        offset_right = int(
            image.shape[1] - obj_pose[0] - self.config.INPUT_SIZE / 2)
        offset_bottom = int(
            image.shape[0] - obj_pose[1] - self.config.INPUT_SIZE / 2)
        ...
        padded_image = np.lib.pad(
            image,
            ((pad_up, pad_bottom), (pad_left, pad_right), (0, 0)),
            'constant',
        ...
        cropped_image = padded_image[
            offset_up + pad_up:
            offset_up + pad_up + self.config.INPUT_SIZE,
            offset_left + pad_left:
            offset_left + pad_left + self.config.INPUT_SIZE
        ]
