...
class Prob3dPose:
    ...
    def transform_joints(pose_2d, visible_joints):
        """
        Transform the set of joints according to what the probabilistic
        model expects as input.

        It returns the new set of joints of each of the people and the set
        of weights for the joints.
        """
    ...
    def affine_estimate(
            self, w, depth_reg=0.085, weights=np.zeros((0, 0, 0)),
            scale=10.0, scale_mean=0.0016 * 1.8 * 1.2, scale_std=1.2 * 0,
            cap_scale=-0.00129):
        """
        ...
        res, a, r = pick_e(
            w, e2, t_m, self.cam, s, weights=weights,
